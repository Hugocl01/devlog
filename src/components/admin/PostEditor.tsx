...
import {
  Loader2, Eye, EyeOff, Save, Tag, X, Trash2, Calendar,
...
export default function PostEditor({ mode, initialData, users = [] }: Props) {
  const storageKey = `post-draft-${initialData?.slug ?? "new"}`;
  ...
  const [tags, setTags] = useState<string[]>(saved?.tags ?? initialData?.tags ?? []);
  ...
  const [coverImage, setCoverImage] = useState(saved?.coverImage ?? initialData?.coverImage ?? "");
  ...
  useEffect(() => {
    ...
    autoSaveTimer.current = setTimeout(() => {
      localStorage.setItem(storageKey, JSON.stringify({ title, description, content, tags, draft, scheduledAt, coverImage, metaTitle, canonical }));
      setAutoSaveStatus("saved");
      setAutoSavedAt(new Date());
    }, 30_000);
    return () => { if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current); };
  }, [title, description, content, tags, draft, scheduledAt, coverImage, metaTitle, canonical]);
  ...
  // Inserta sintaxis Markdown respetando la selección activa
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = mode === "create" ? "/api/admin/posts" : `/api/admin/posts/${initialData!.slug}`;
    ...
    toast.promise(p, {
      loading: mode === "create" ? "Creando post..." : "Guardando cambios...",
      success: mode === "create" ? "Post creado correctamente" : "Cambios guardados",
    ...
    try {
      ...
      if (mode === "create") window.location.href = `/admin/posts/${data.slug}/edit`;
    } catch {
  ...
  const handleDelete = async () => {
    ...
    const p = fetch(`/api/admin/posts/${initialData!.slug}`, { method: "DELETE" })
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {mode === "create" ? "Crear post" : "Guardar"}
          </button>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className={field}>
            ...
            <textarea
              ...
              placeholder="Descripción breve del post (aparece en listados y SEO)"
              rows={3}
            />
          </div>
            ...
            {!preview && (
              <div className="flex flex-wrap gap-0.5 rounded-t-lg border border-b-0 border-border/60 bg-secondary/30 px-2 py-1.5">
                {([
                  ...
                  { icon: null }, // divider
                  { icon: <Bold className="h-3.5 w-3.5" />, title: "Negrita (Ctrl+B)", fn: () => insertMarkdown("**", "**", "negrita") },
                  { icon: <Italic className="h-3.5 w-3.5" />, title: "Cursiva (Ctrl+I)", fn: () => insertMarkdown("*", "*", "cursiva") },
                  { icon: null },
          ...
          <div className={field}>
            ...
            <div className="flex gap-1.5">
              <div className="relative flex-1">
                <Tag className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 h-3 w-3 text-muted-foreground/50" />
                <input
                  type="text"
              ...
              <button
                type="button"
                onClick={addTag}
                className="h-9 px-3 rounded-full border border-border/60 bg-secondary/30 text-xs text-muted-foreground hover:text-foreground hover:bg-secondary/60 transition-colors"
              >
                Añadir
              </button>
            </div>
      ...
      <div className="rounded-xl border border-border/60 overflow-hidden">
        ...
        {seoOpen && (
          <div className="p-4 flex flex-col gap-4 border-t border-border/40">
            <div className={field}>
              <label htmlFor="meta-title" className={label}>Meta title <span className="normal-case font-normal text-muted-foreground/60">(override del título en buscadores)</span></label>
              <input
                id="meta-title"
                type="text"
                className={input}
                ...
                placeholder={title || "Deja vacío para usar el título del post"}
                maxLength={60}
              />
            ...
            <div className={field}>
              <label htmlFor="canonical" className={label}>URL canónica <span className="normal-case font-normal text-muted-foreground/60">(si el contenido existe en otra URL)</span></label>
              <input
                id="canonical"
                type="url"
                className={input}
            ...
            <div className={field}>
              ...
              <div className="rounded-lg border border-border/60 bg-background p-4 font-sans">
                ...
                <p className="text-[12px] text-[#006621] dark:text-[#34a853] mt-0.5 truncate">
                  {typeof window !== "undefined" ? window.location.origin : "https://tusitio.com"}/blog/{initialData?.slug ?? "slug-del-post"}
                </p>
                <p className={cn("text-[13px] text-[#545454] dark:text-[#bdc1c6] mt-0.5 line-clamp-2 leading-snug", !description && "italic opacity-50")}>
                  {description || "Descripción del post (rellena el campo descripción)"}
                </p>
              </div>
